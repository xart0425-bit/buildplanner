...
| 🤖 **모델 선택** | Claude · Gemini · OpenAI 중 원하는 키를 [설정]에 넣으면 그 모델로 분석합니다. |
| 🌐 **분석 언어** | 영어(기본) · 한국어 · 일본어 · 중국어 · 프랑스어 · 러시아어로 결과물을 생성합니다. |
...
**LLM 키는 `.env`에 넣지 않아도 됩니다.** 앱 우측 상단 **[설정]** 에서 Claude(Anthropic), Gemini,
OpenAI 중 하나의 키를 입력하면 브라우저 로컬 저장소에만 보관되고 요청 헤더로 전달됩니다.
여러 개를 넣으면 **Claude → Gemini → OpenAI** 순으로 사용합니다.

같은 화면에서 **분석 언어**(영어 기본 / 한국어 / 일본어 / 중국어 / 프랑스어 / 러시아어)를 고를 수 있습니다.
계획서·스펙·역설계 보고서가 그 언어로 작성됩니다. 앱 화면 자체의 언어는 바뀌지 않습니다.
...
| `ANTHROPIC_API_KEY` / `GEMINI_API_KEY` / `OPENAI_API_KEY` | 아니오 | 서버 공용 LLM 키. 없으면 사용자가 [설정]에서 각자 입력 |
| `LLM_MODEL` | 아니오 | 모델 지정 (기본: `claude-opus-5` / `gemini-2.5-flash` / `gpt-4o-mini`) |
